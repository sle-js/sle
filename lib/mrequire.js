...
function loadCorePackage(name, names) {
    ...
    if (dirExists(fullPathName)) {
        ...
        try {
            ...
            ChildProcess.execSync(`git clone --quiet -b ${names[2]} --single-branch https://github.com/graeme-lockley/mn-${names[1]}.git ${names[2]} 2>&1 >> /dev/null`, {cwd: libraryPath(names)});
